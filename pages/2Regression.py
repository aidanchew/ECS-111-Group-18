...
def run_regression(X_data, y_reg, condition_name):
    ...
    if "Condition 1" in condition_name:
        st.markdown("""
For Condition 1 the three models showed a clear gap between linear and tree based approaches. Linear Regression had the weakest R² of the three because the relationship between features and GPA is not actually a straight line. Decision Tree did better at picking up non-linear patterns but its R² was held back by overfitting since a single tree memorizes the training data and struggles to generalize. Random Forest was the best performer here with the lowest RMSE and the highest R². That makes sense because averaging across many trees smooths out the overfitting that hurt the Decision Tree on its own. The fact that the tree based models beat Linear Regression tells us there are non-linear relationships and feature interactions in the data that a straight line cannot capture.
""")
    else:
        st.markdown("""
For Condition 2 every model dropped in performance once we removed the academic features like study time, failures, and absences. This is the most important finding from the regression page. The drop confirms that academic behavior is the strongest signal for predicting GPA and that demographic or family features alone do not carry enough information. Random Forest still came out as the best of the three but its R² fell significantly compared to Condition 1, which means even the most flexible model cannot make up for missing the most predictive features. Linear Regression and Decision Tree both struggled here as well. The takeaway is that non-academic features have some predictive value but they are not enough on their own to reliably predict final GPA.
""")
