...
st.markdown("""
The Pass/Fail Page was ran using 2 classification models: **Logistic Regression** and **Random Forest**. To evaluate the model, we will be observing **Accuracy, F1 Score, ROC/AUC, and Cross-Validation F1.**  

Additionally, we included a **SHAP feature importance plot** under each Random Forest to show which features had the largest impact on the model’s predictions. 
...
def run_pass_fail(X_data, y_clf, condition_name):
    ...
    if "Condition 1" in condition_name:
        ...
        st.markdown("""
**Insight:**  Both models were comparable in scores, with Logistic being slightly better.

1) Logistic Regression came out on top with an **Accuracy of 79.22%**, an **F1 Score of 0.8644**

2) Random Forest had a **higher ROC AUC of 0.8137** than Logistic Regression. Therefore, **Random Forest is actually better at separating the two classes** when you look across all probability thresholds

3) The **Cross-Val F1 for Random Forest** at 0.8508 is also **higher than the test F1**, which suggests the **test set** just happened to be a slightly **harder split**.

4) The SHAP plot should be dominated by failures_mat just like the EDA was warning us about.

**Takeaway:** Both models are predicting pass/fail well. 
""")
    else:
        st.write("")
        st.subheader("📋Analysis")
        st.markdown("""
**Insight:** The Gap between both models have been closed. 
                    
1) Both models' metrics are essentially similar. 
                    
2) Random Forest Cross-Val. F1 is close to Condition 1 Score, meaning the **model is finding a consistent signal** even without the academic features
                    
3) Both models are still hitting around **72-74% accuracy**, which beats just guessing pass every time at 71%.
                    
4) The SHAP plot here should rely more on study time, parental education, and the failures_mat that survives the academic cut.
                    
**Takeaway**: The pass/fail task is performing better than the regression in Condition 2, which is expected since predicting a binary outcome is a much easier ask than predicting an exact GPA number.
""")
...
with col5:
    st.page_link("pages/5GPAGroups.py", label="Next Page", icon="▶️", use_container_width="content")